    ...
    <div className="w-52 bg-gradient-to-b from-amber-800 to-amber-900 text-white flex flex-col">
      ...
      <nav className="p-2 flex-1">
        ...
        <div className="flex flex-col gap-2">
          ...
          <NavLink
            to="/NhapKhoThanhPham"
            onClick={() => handleSetActive('NhapKhoThanhPham')}
            className={({ isActive }) =>
              `flex items-center gap-2 px-3 py-2 rounded-lg transition-colors text-sm ${
                isActive || activeMenu === 'NhapKhoThanhPham' ? 'bg-amber-600' : 'hover:bg-amber-700'
              }`
            }
          >
            <Archive size={16} />
            <span>Nhập kho TP</span>
          </NavLink>

          <NavLink
            to="/XuatKhoThanhPham"
            onClick={() => handleSetActive('XuatKhoThanhPham')}
            className={({ isActive }) =>
              `flex items-center gap-2 px-3 py-2 rounded-lg transition-colors text-sm ${
                isActive || activeMenu === 'XuatKhoThanhPham' ? 'bg-amber-600' : 'hover:bg-amber-700'
              }`
            }
          >
            <Truck size={16} />
            <span>Xuất kho TP</span>
          </NavLink>
        </div>
      </nav>
